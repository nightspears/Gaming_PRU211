...
public class LevelGeneration : MonoBehaviour
{
    ...
    private float timeBetweenMoves = 1f;  // Adjust this value to control the delay between moves
    ...
    {
        // chay qua tung o mot den gen map ( luu y grid tu -30 den 30)
        for (float x = -30; x <= 30; x += moveAmount)
        {
            for (float y = MinY; y <= 30; y += moveAmount)
            {
